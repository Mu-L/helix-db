...
async fn pull_from_local_instance(project: &ProjectContext, instance_name: &str) -> Result<()> {
    print_status("PULL", &format!("Pulling queries from local instance '{}'", instance_name));
    ...
    if !container_dir.exists() {
        return Err(eyre::eyre!(
            "Instance '{}' has not been built yet. Run 'helix build {}' first.",
...
async fn pull_from_cloud_instance(_project: &ProjectContext, instance_name: &str, instance_config: InstanceInfo<'_>) -> Result<()> {
    print_status("CLOUD", &format!("Pulling queries from cloud instance '{}'", instance_name));
    
    let cluster_id = instance_config.cluster_id()
        .ok_or_else(|| eyre::eyre!("Cloud instance '{}' must have a cluster_id", instance_name))?;
    
    print_status("DOWNLOAD", &format!("Downloading from cluster: {}", cluster_id));
