use eyre::Result;
use crate::{CloudAction, utils::{print_status, print_success, print_warning}};

pub async fn run(action: CloudAction) -> Result<()> {
    match action {
        CloudAction::Login => login().await,
        CloudAction::Logout => logout().await,
        CloudAction::CreateKey { cluster } => create_key(&cluster).await,
    }
}
...
async fn create_key(cluster: &str) -> Result<()> {
    print_status("API_KEY", &format!("Creating API key for cluster: {}", cluster));
