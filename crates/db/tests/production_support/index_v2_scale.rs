...
pub(super) async fn run_secondary_text_tenant() {
    ...
    let db = open_seeded_unscoped(
        "index-v2-production-secondary-text-scale",
...
pub(super) async fn run_vector() {
    ...
    run_vector_fixture(
        "index-v2-production-vector-scale",
        NonZeroUsize::new(ENTITY_COUNT).expect("release entity count is positive"),
    )
    .await;
}
...
pub(super) async fn run_traversal_vector_prefilter() {
    ...
    let token = ProcessLocalDatabaseToken::new("index-v2-traversal-vector-prefilter-scale")
...
pub(super) async fn run_traversal_vector_prefilter_1m() {
    ...
    let db = HelixDB::open(HelixDbSource::Disk {
        ...
        database: "index-v2-traversal-vector-prefilter-1m".to_string(),
    })
    .await
...
pub(super) async fn run_blocked_limits() {
    ...
    let token = ProcessLocalDatabaseToken::new("index-v2-production-blocked-limit-scale")
    ...
    for (family, create, drop) in cases {
        ...
        assert_eq!(
            blocker_code,
            crate::index_v2::IndexOperationBlockerCode::OversizedEntity
        );
