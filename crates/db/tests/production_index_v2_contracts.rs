...
#[tokio::test]
async fn index_v2_writer_migration_requirements_are_typed() {
...
#[test]
fn index_v2_failpoint_abort_action_terminates_process() {
    const CHILD_ENV: &str = "HELIX_INDEX_OUTBOX_ABORT_PROBE_CHILD";
    if std::env::var_os(CHILD_ENV).is_some() {
        db::production_coverage::index_v2_failpoint_abort_probe();
    }

    let status = Command::new(std::env::current_exe().expect("test executable path resolves"))
        .args([
            "--exact",
            "index_v2_failpoint_abort_action_terminates_process",
            "--nocapture",
        ])
        .env(CHILD_ENV, "1")
...
#[test]
fn index_v2_typed_boundaries_fail_closed() {
    db::production_coverage::index_v2_typed_boundary_contracts();
}

/// Proves Active text serving reads reject every cross-owned durable row.
#[tokio::test]
async fn index_v2_text_serving_reads_fail_closed() {
    db::production_coverage::index_v2_text_serving_contracts().await;
}

/// Proves state-only Active text retirement validates before atomic staging.
#[tokio::test]
async fn index_v2_active_text_retirement_fails_closed() {
    db::production_coverage::index_v2_active_text_retirement_contracts().await;
}
