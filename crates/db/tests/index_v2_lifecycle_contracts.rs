...
async fn index_v2_deterministic_lifecycle_matrix() {
    db::index_v2_lifecycle_testing::run_deterministic_lifecycle_contracts().await;
}

/// Proves mutations converge during backfill and unique validation can recover.
#[test]
fn index_v2_backfill_mutations_and_unique_retry_converge() {
    ...
    std::thread::Builder::new()
        .name("index-v2-lifecycle-mutation".to_string())
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_multi_thread()
                ...
                .block_on(
                    db::index_v2_lifecycle_testing::run_deterministic_lifecycle_mutation_contracts(
                    ),
                );
        })
...
#[tokio::test(flavor = "multi_thread")]
async fn index_v2_concurrent_create_converges_for_every_family() {
    db::index_v2_lifecycle_testing::run_deterministic_lifecycle_race_contracts().await;
}

/// Proves every family resumes after two failures at the same durable boundary.
#[tokio::test(flavor = "multi_thread")]
async fn index_v2_repeated_recoverable_errors_resume_exactly_once() {
    db::index_v2_lifecycle_testing::run_deterministic_lifecycle_fault_contracts().await;
}
