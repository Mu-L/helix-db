    ...
    for (ordinal, definition) in definitions.into_iter().enumerate() {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!("index-v2-lifecycle-repeated-fault-{ordinal}"),
            },
            DbConfig::new(),
...
async fn run_concurrent_create_convergence() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-concurrent-create".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
...
async fn run_conflicting_create_convergence() {
    ...
    for (ordinal, (left_definition, right_definition)) in conflicts.into_iter().enumerate() {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!("index-v2-lifecycle-conflicting-create-{ordinal}"),
            },
            DbConfig::new(),
            LifecycleTestScheduling::Explicit,
        )
...
async fn run_activation_and_drop_serialization() {
    for drop_wins in [true, false] {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!(
                    "index-v2-lifecycle-activation-drop-{}",
...
async fn run_active_drop_and_mutation_serialization() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-active-drop-mutation".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
    ...
    let mutations = crate::index_v2::secondary::load_mutation_set(&transaction, scope)
        .await
...
async fn run_automatic_worker_convergence() {
    ...
    .enumerate()
    {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!("index-v2-lifecycle-automatic-worker-{ordinal}"),
            },
            DbConfig::new(),
...
async fn run_secondary_mutation_interleaving() {
    ...
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-secondary-mutations".to_string(),
        },
        config,
        LifecycleTestScheduling::Explicit,
    )
...
async fn run_secondary_edge_mutation_interleaving() {
    ...
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-secondary-edge-mutations".to_string(),
        },
        config,
        LifecycleTestScheduling::Explicit,
    )
...
async fn run_every_family_public_mutation_interleaving() {
    ...
    .enumerate()
    {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!("index-v2-lifecycle-public-mutation-{ordinal}"),
            },
            DbConfig::new(),
            LifecycleTestScheduling::Explicit,
        )
...
async fn run_partitioned_search_edge_tenant_moves() {
        ...
        .enumerate()
    {
        let db = HelixDB::open_for_index_v2_lifecycle_testing(
            HelixDbSource::InMemory {
                database: format!("index-v2-lifecycle-partitioned-edge-{family_ordinal}"),
            },
            DbConfig::new(),
            LifecycleTestScheduling::Explicit,
        )
...
async fn assert_build_delta_count_at_least(
    ...
    let record =
        crate::index_v2::repository::load_index_record(writer, scope, &definition.identity())
            .await
...
async fn mutate_source(
    ...
    let mutations = crate::index_v2::secondary::load_mutation_set(&transaction, scope).await?;
    crate::index_v2::secondary::maintain_entity(
        &transaction,
        scope,
        &mutations,
...
async fn assert_equality<const N: usize>(
    ...
    let actual = crate::index_v2::secondary::lookup_active_equality_generation(
        db.lifecycle_test_writer_db()
            .expect("equality lookup has writer storage"),
        &handle,
...
async fn run_family_shapes() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-family-shapes".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
    ...
    for definition in family_shapes() {
        ...
        let active = crate::index_v2::repository::load_index_record(
            db.lifecycle_test_writer_db()
        ...
        let dropped = crate::index_v2::repository::load_index_record(
            db.lifecycle_test_writer_db()
                .expect("shape writer storage is available"),
            DataScope::LegacyUnscoped,
            &identity,
...
async fn run_create_drop_state_matrix() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-create-drop-matrix".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
    ...
    for (family_ordinal, definition) in representative_families().into_iter().enumerate() {
            ...
            .enumerate()
        {
            ...
            let db = HelixDB::open_for_index_v2_lifecycle_testing(
                HelixDbSource::InMemory {
                    database: format!(
                        "index-v2-lifecycle-building-drop-{family_ordinal}-{checkpoint_ordinal}"
                    ),
                },
                config,
                LifecycleTestScheduling::Explicit,
...
async fn run_active_drop_snapshot_survival() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-active-drop-snapshot-survival".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
    ...
    assert_eq!(
        crate::index_v2::secondary::lookup_active_equality_generation(
            snapshot.as_ref(),
            &active_handle,
...
async fn run_active_vector_drop_snapshot_survival() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-active-vector-drop-snapshot-survival".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
...
async fn run_scope_isolation() {
    let db = HelixDB::open_for_index_v2_lifecycle_testing(
        HelixDbSource::InMemory {
            database: "index-v2-lifecycle-scope-isolation".to_string(),
        },
        DbConfig::new(),
        LifecycleTestScheduling::Explicit,
    )
