...
pub(crate) async fn compact_once(
    ...
    let index_v2::IndexV2MetadataValue::TextCompactionPointer(pointer_value) =
    ...
    let index_v2::IndexStateV2::Active { physical, .. } = record.state() else {
        drop(snapshot);
        delete_exact_pointer(db, &pointer.key, &pointer.value).await?;
        return Ok(true);
    };
    let index_v2::PhysicalGeneration::Text { generation } = physical else {
        return Err(corruption(
            "text compaction pointer resolves to another physical family",
        ));
    };
    let index_v2::ValidatedDynamicIndexDefinition::Text(definition) = record.definition() else {
        return Err(corruption(
                ...
                scoped_key(
                    ...
                    index_keys::ScopedKey::TextEntityState(index_keys::TextEntityStateKey {
                        ...
                        entity: index_keys::IndexEntity {
                            ...
                            id: index_v2::IndexEntityId::new(entity_id),
                        },
                    }),
                )
            })
    ...
    if select_tier(page.entries(), limits).is_some() {
        transaction.put(
            ...
            index_values::encode_metadata_value(
                &index_v2::IndexV2MetadataValue::TextCompactionPointer(
                    index_v2::TextCompactionPointerValue { revision },
                ),
            ),
        )?;
    } else {
...
mod tests {
    ...
    async fn active_page_compaction_replaces_inputs_and_consumes_pointer() {
        ...
        let definition =
            index_v2::ValidatedTextIndexDefinition::try_from_runtime(&runtime).unwrap();
        let index_id = index_v2::IndexId::initial();
        let generation = index_v2::IndexGenerationId::initial();
        let record = index_v2::IndexRecordV2::building(
            index_id,
            index_v2::ValidatedDynamicIndexDefinition::Text(definition.clone()),
            index_v2::IndexRevision::initial(),
            index_v2::PhysicalGeneration::Text { generation },
            ...
            db.put(
                ...
                index_values::encode_text_entity_state(&
                    work::TextEntityStateValue {
                        ...
                        entity_kind: index_v2::IndexElementKind::Node,
                        entity_id: index_v2::IndexEntityId::new(entity_id),
