...
#[cfg(test)]
use crate::index_v2::IndexStateV2;
use crate::index_v2::{
...
pub(in crate::index_v2) enum TextMutationTargetOrdinal {
...
impl TextMutationSet {
    ...
    #[cfg(test)]
    pub(in crate::index_v2) const fn catalog_entry_count(&self) -> usize {
        self.targets.len() + self.active_handles.len()
    }

    /// Classifies one same-snapshot canonical text record.
    pub(in crate::index_v2) fn include_catalog_entry(
        &mut self,
        entry: crate::index_v2::mutation_catalog::MutationCatalogEntry<'_>,
    ) -> Result<TextMutationTargetOrdinal> {
        match entry {
            crate::index_v2::mutation_catalog::MutationCatalogEntry::Building(record) => {
            ...
            crate::index_v2::mutation_catalog::MutationCatalogEntry::Active { record, handle } => {
                if !matches!(handle, crate::index_v2::ActiveIndexHandle::Text { .. })
                    || !matches!(
                        record.definition(),
...
pub(crate) async fn load_mutation_set(
    ...
    while let Some(row) = rows.next().await? {
        ...
        let entry = match active_handle.as_ref() {
            Some(handle) => crate::index_v2::mutation_catalog::MutationCatalogEntry::Active {
                record: &record,
                handle,
            },
            None => crate::index_v2::mutation_catalog::MutationCatalogEntry::Building(&record),
        };
...
pub(super) async fn prepare_text_build_deltas(
    ...
    let routes = crate::index_v2::mutation_catalog::RoutedMutationTargets::Owned(
        mutations
            .targets
            .iter()
            .enumerate()
            ...
            .map(|(ordinal, _)| {
                crate::index_v2::mutation_catalog::MutationRouteTarget::TextBuilding(ordinal)
            })
            .collect(),
    );
...
pub(super) async fn prepare_text_build_deltas_in_batch(
    ...
    scope: DataScope,
    mutations: &TextMutationSet,
    routes: &crate::index_v2::mutation_catalog::RoutedMutationTargets<'_>,
    entity: TextEntityMutation<'_>,
...
async fn prepare_text_build_deltas_from(
    ...
    for ordinal in routes.iter().filter_map(|target| match target {
        ...
        crate::index_v2::mutation_catalog::MutationRouteTarget::Secondary(_)
        | crate::index_v2::mutation_catalog::MutationRouteTarget::Vector(_)
        | crate::index_v2::mutation_catalog::MutationRouteTarget::TextActive(_) => None,
    }) {
