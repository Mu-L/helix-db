...
pub(crate) async fn prepare_active_text_epoch(
    ...
    routes: &index_v2::mutation_catalog::MutationRouteCatalog,
    ...
    for graph in &graphs {
        ...
        for ordinal in graph_routes.iter().filter_map(|target| match target {
            index_v2::mutation_catalog::MutationRouteTarget::TextActive(ordinal) => Some(ordinal),
            index_v2::mutation_catalog::MutationRouteTarget::Secondary(_)
            | index_v2::mutation_catalog::MutationRouteTarget::Vector(_)
            | index_v2::mutation_catalog::MutationRouteTarget::TextBuilding(_) => None,
        }) {
    ...
    writes.push(PreparedRow {
        ...
        value: index_values::encode_metadata_value(
            &index_v2::IndexV2MetadataValue::TextCompactionPointer(
                index_v2::TextCompactionPointerValue {
...
struct AppendSplitRequest<'a> {
    ...
    split: work::SplitRef,
    next_revision: index_v2::TextManifestRevision,
    limits: ActiveTextMutationLimits,
        ...
        let loser_value =
            index_values::encode_manifest_root(&
                work::TextManifestRootValue::try_new(
                    ...
                    generation,
                    partition.clone(),
                    index_v2::TextManifestRevision::new(2).unwrap(),
                    0,
                    0,
                )
