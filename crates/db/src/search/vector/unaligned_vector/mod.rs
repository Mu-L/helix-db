//! Borrowed byte-backed vector views and their active codec contract.
//!
//! Vector rows remain in their deployed physical representation. This module
//! provides checked views over those bytes so distance kernels can consume
//! current `f32` payloads without alignment assumptions or format rewrites.
//! Future binary and binary-quantized codecs stay feature-gated and are not
//! descriptor-bindable active formats.
...
pub use simhash::{SimHash, SimHashError, SimHasher};
...
pub trait UnalignedVectorCodec: std::borrow::ToOwned + Sized {
    ...
    /// Compute SimHash for this vector using the provided hasher
    ///
    /// This enables efficient similarity filtering in layer 0 search by computing
    /// a locality-sensitive hash that can be checked before loading full vectors.
    ///
    /// # Arguments
    /// * `vec` - The vector to hash
    /// * `hasher` - The SimHasher with precomputed random hyperplanes
    ///
    /// # Returns
    /// A 64-bit SimHash code that can be compared with query SimHash.
    ///
    /// # Errors
    ///
    /// Returns [`SimHashError::DimensionMismatch`] when the decoded vector
    /// length differs from the dimension bound to `hasher`.
    fn compute_simhash(
