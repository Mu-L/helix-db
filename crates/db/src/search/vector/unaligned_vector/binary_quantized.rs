...
impl UnalignedVectorCodec for BinaryQuantized {
    fn from_bytes(bytes: &[u8]) -> Result<Cow<'_, UnalignedVector<Self>>, SizeMismatch> {
        ...
        if rem == 0 {
            ...
            Ok(Cow::Borrowed(unsafe {
                transmute::<&[u8], &UnalignedVector<Self>>(bytes)
            }))
        } else {
            Err(SizeMismatch {
                vector_codec: "binary quantized",
                rem,
            })
        }
    }
    ...
    fn iter(vec: &UnalignedVector<Self>) -> impl ExactSizeIterator<Item = f32> + '_ {
        BinaryQuantizedIterator {
            ...
            iter: vec.vector.as_chunks::<QUANTIZED_WORD_BYTES>().0.iter(),
        }
    }
    ...
    fn compute_simhash(
        ...
        // For binary quantized vectors, we already have efficient iteration
        // Iterator produces -1.0 or 1.0 values which we hash directly
        hasher.hash_from_iter(vec.iter())
    }
}
...
pub struct BinaryQuantizedIterator<'a> {
    ...
    iter: Iter<'a, [u8; QUANTIZED_WORD_BYTES]>,
}
...
impl Iterator for BinaryQuantizedIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.current_iteration >= QUANTIZED_WORD_BITS {
            ...
            self.current_element = QuantizedWord::from_ne_bytes(*bytes);
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        (
            low * QUANTIZED_WORD_BITS + rem,
            high.map(|h| h * QUANTIZED_WORD_BITS + rem),
        )
    }
}
