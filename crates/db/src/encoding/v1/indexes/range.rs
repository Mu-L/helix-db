//! Canonical V1 ordered secondary-index key codecs.
//!
...
mod tests {
    use super::*;
    use crate::encoding::{indexes::IndexKey, keys::DataKeyKind};

    const PROP: PropertyHash = [1, 2, 3, 4];

    #[test]
    fn range_index_asc_layout_round_trips() {
        let key = DataKeyKind::PropertyIndex(IndexKey::Range(RangeIndexKey::new(
        ...
        let DataKeyKind::PropertyIndex(IndexKey::Range(parsed)) = parsed else {
    ...
    fn range_index_desc_layout_decodes_original_value() {
        ...
        let DataKeyKind::PropertyIndex(IndexKey::Range(parsed)) =
    ...
    fn edge_range_layouts_encode_range_then_edge_direction() {
        let out_desc = DataKeyKind::PropertyIndex(IndexKey::EdgeRange(EdgeRangeIndexKey::new(
            EdgeDirection::Out,
            EdgeRangeIndexDirection::Desc,
            7,
            PROP,
        ...
        .to_bytes();
        let in_asc = DataKeyKind::PropertyIndex(IndexKey::EdgeRange(EdgeRangeIndexKey::new(
            EdgeDirection::In,
        ...
        let DataKeyKind::PropertyIndex(IndexKey::EdgeRange(parsed)) =
            DataKeyKind::parse_from_slice(&out_desc).unwrap()
        else {
    ...
    fn range_keys_sort_by_direction() {
        let asc_a = DataKeyKind::PropertyIndex(IndexKey::Range(RangeIndexKey::new(
            RangeIndexDirection::Asc,
            PROP,
            Cow::Borrowed("a"),
            1,
        ...
        .to_bytes();
        let asc_b = DataKeyKind::PropertyIndex(IndexKey::Range(RangeIndexKey::new(
            RangeIndexDirection::Asc,
            PROP,
            Cow::Borrowed("b"),
            2,
        ...
        .to_bytes();
        let desc_a = DataKeyKind::PropertyIndex(IndexKey::Range(RangeIndexKey::new(
            RangeIndexDirection::Desc,
            PROP,
            Cow::Borrowed("a"),
            1,
        ...
        .to_bytes();
        let desc_b = DataKeyKind::PropertyIndex(IndexKey::Range(RangeIndexKey::new(
            RangeIndexDirection::Desc,
            PROP,
            Cow::Borrowed("b"),
            2,
    ...
    fn global_edge_range_index_key_has_exact_layout_and_round_trips() {
        ...
        let desc = DataKeyKind::PropertyIndex(IndexKey::GlobalEdgeRange(
            GlobalEdgeRangeIndexKey::new(RangeIndexDirection::Desc, PROP, Cow::Borrowed("a\0z"), 2),
        ));
        let encoded = desc.to_bytes();
        let DataKeyKind::PropertyIndex(IndexKey::GlobalEdgeRange(parsed)) =
            DataKeyKind::parse_from_slice(&encoded).unwrap()
        else {
    ...
    fn edge_range_parser_rejects_wrong_prefixes_and_malformed_values() {
        ...
        let key = DataKeyKind::PropertyIndex(IndexKey::EdgeRange(EdgeRangeIndexKey::new(
            EdgeDirection::In,
            EdgeRangeIndexDirection::Desc,
            1,
            PROP,
