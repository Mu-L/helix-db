//! Edge equality-union restriction by range intersections.
...
pub(super) fn simplify(plan: &ir::EdgeAccessPlan) -> AccessSetPlanRewrite<ir::EdgeAccessPlan> {
    ...
    let restriction = replacement::apply_intersection_restriction(
        plans,
        find_restriction,
        |source| matches!(source.as_ref(), ir::EdgeAccessPlan::Empty),
        super::super::super::edge_intersection_from_sources,
...
pub(super) fn has_candidate(plan: &ir::EdgeAccessPlan) -> bool {
    ...
    let mut range_buckets: BTreeMap<digest::PlanDigest, Vec<EdgeRangeBucketEntry<'_>>> =
        BTreeMap::new();
    plans.iter().enumerate().for_each(|(index, source)| {
        if let Some((digest, entry)) =
            edge_range_bucket_entry("edge_equality_range_property:v1", index, source)
        {
            range_buckets.entry(digest).or_default().push(entry);
        }
    });
    plans.iter().any(|source| {
        let ir::EdgeAccessPlan::Union(children) = source.as_ref() else {
            return false;
        };
        ...
        let digest = scoped_property_digest(
            "edge_equality_range_property:v1",
            &union_key.label,
            &union_key.property,
        );
...
fn find_restriction(
    slots: &[Option<ir::EdgeAccessSourcePlan>],
) -> replacement::RangeRestrictionMatch<ir::EdgeAccessSourcePlan> {
    let mut range_buckets: BTreeMap<digest::PlanDigest, Vec<EdgeRangeBucketEntry<'_>>> =
        BTreeMap::new();
    ...
    for (union_index, union) in slots.iter().enumerate().filter_map(|(index, source)| {
        let ir::EdgeAccessPlan::Union(children) = source.as_ref()?.as_ref() else {
            return None;
        };
        Some((index, children))
    }) {
...
fn equality_union_can_be_restricted_by_range(
    ...
    children.iter().all(|child| {
        let Some((equality_key, value)) = edge_literal_equality_parts(child) else {
            return false;
        };
...
fn equality_union_key(
    ...
    let (first, _) = edge_literal_equality_parts(first_child)?;
    rest.iter()
        .all(|child| edge_literal_equality_parts(child).is_some_and(|(key, _)| key == first))
        .then_some(first)
}
...
fn restrict_equality_union_by_range(
    ...
    range_key: &catalog::ScopedPropertyDirectionKey,
    range: &ir::IndexRange,
) -> replacement::EqualityUnionRangeRestriction<ir::EdgeAccessSourcePlan> {
    ...
    replacement::EqualityUnionRangeRestriction::Restricted(edge_source_from_union_candidates(
        retained,
    ))
}
