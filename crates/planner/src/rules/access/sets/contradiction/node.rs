//! Node access contradiction proofs.

use super::super::*;

pub(super) fn has_static_contradiction(plan: &ir::NodeAccessPlan) -> bool {
    match plan {
        ir::NodeAccessPlan::Intersect(plans) => plans.iter().enumerate().any(|(index, left)| {
...
fn is_statically_empty(plan: &ir::NodeAccessPlan) -> bool {
    match plan {
        ir::NodeAccessPlan::Empty => true,
        ir::NodeAccessPlan::Intersect(_) => has_static_contradiction(plan),
        ir::NodeAccessPlan::Union(plans) => plans
            .iter()
...
fn plans_contradict(left: &ir::NodeAccessPlan, right: &ir::NodeAccessPlan) -> bool {
    if is_statically_empty(left) || is_statically_empty(right) {
        return true;
    }
    match (left, right) {
        (ir::NodeAccessPlan::Union(children), other) => children
            .iter()
            .all(|child| plans_contradict(child.as_ref(), other)),
        (other, ir::NodeAccessPlan::Union(children)) => children
            .iter()
            .all(|child| plans_contradict(other, child.as_ref())),
        (ir::NodeAccessPlan::Intersect(children), other) => children
            .iter()
            .any(|child| plans_contradict(child.as_ref(), other)),
        (other, ir::NodeAccessPlan::Intersect(children)) => children
            .iter()
...
fn literal_equalities_conflict(left: &ir::NodeAccessPlan, right: &ir::NodeAccessPlan) -> bool {
...
fn literal_equality_range_conflict(
    equality: &ir::NodeAccessPlan,
    range: &ir::NodeAccessPlan,
) -> bool {
