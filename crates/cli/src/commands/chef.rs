...
This project authors HelixDB queries with the **TypeScript DSL** (`@helix-db/helix-db`) — typed builder functions, not hand-written JSON. Never write Rust `.hx` files; there is no compile step. Queries live as builder modules under `web/src/lib/queries/*.ts`; a small server-only runner (`web/src/lib/helix.ts`) serializes them and POSTs to the local DB at `http://localhost:6969/v2/query`. Raw query JSON (`helix query dev --file/--json`) stays available as a fallback for debugging. The frontend that consumes these queries is a Next.js app under `web/` (App Router, TypeScript, Tailwind) — see `<frontend>`.
</environment>
...
{seed_step}
8. **Test each query against the running DB** before wiring the UI. Run a one-off script: `web/scripts/<name>.ts` that imports the builder + `runQuery` and prints the result — `cd web && npx tsx scripts/<name>.ts`. (Quick raw-JSON spot check: `console.log(myQuery().toQueryJson(params, values))` and pipe into `helix query dev --json "$(...)"`.) Fix and retry until each returns the expected shape.
...
**Serialize / run:** `myQuery().toQueryRequest(params, values)` (object) or `.toQueryJson(params, values)` (string body). No-param queries take no args: `myQuery().toQueryJson()`. Use `bigint` (`25n`) or `i64(...)` for large integers; serialize with the SDK helpers, never raw `JSON.stringify`.
...
**Fallback — raw JSON.** Query JSON sent to `helix query dev --json/--file` is the fallback for quick debugging. The DSL emits exactly this JSON; its encoding rules (tagged `PropertyValue`, lowercase `request_type`, untagged `Project`, etc.) live in the `helix-query-json-dynamic` skill — invoke it if you drop down to raw JSON.
</typescript_dsl_quickref>
...
const HELIX_URL = process.env.HELIX_URL ?? 'http://localhost:6969/v2/query';

export async function runQuery(request: QueryRequest): Promise<unknown> {
...
export async function GET() {
  return NextResponse.json(await runQuery(listContacts().toQueryRequest()));
}

export async function POST(req: NextRequest) {
  const { parameters } = await req.json().catch(() => ({ parameters: {} }));
  const request = addContact().toQueryRequest(addContactParams, {
...
- Parameterized queries: read the client `parameters` from the request body, coerce them to typed values (`String(...)`, `BigInt(...)`, arrays of ids as `bigint[]`), and pass them to `builder().toQueryRequest(params, values)`.
...
- `helix query dev --json '<inline json>'` — run a one-off raw-JSON query (fallback / quick check; pipe `myQuery().toQueryJson(...)` into it).
...
- DO NOT call `JSON.stringify` on a request — use `.toJsonString()` / `.toQueryJson(...)` (bigint-safe).
...
- DO NOT have the browser fetch `http://localhost:6969/v2/query`. Every Helix call goes through a Next.js API route handler in `web/src/app/api/<name>/route.ts`. Server-only.
...
fn chef_metric(
    ...
    let error_message = error_message.map(|message| message.chars().take(500).collect());
    let outcome = if success {
        EventOutcome::Succeeded
    } else {
        EventOutcome::Failed {
            stage: error_stage.map(str::to_string),
            message: error_message.unwrap_or_else(|| "chef operation failed".to_string()),
        }
    };
    ChefEvent {
        run_id: run_id.to_string(),
        phase: ChefPhase::try_from(phase).expect("chef metric phase is predefined"),
        duration_seconds: duration_sec.map(u64::from),
        setup_mode: setup_mode.map(|mode| match mode {
            SetupMode::Automatic => ChefSetupMode::Automatic,
            SetupMode::Manual => ChefSetupMode::Manual,
        }),
        has_custom_intent,
        agent,
        overview_size_bytes,
...
fn default_project_dir() -> Result<PathBuf> {
    Ok(crate::paths::user_home_dir()?.join(DEFAULT_PROJECT_DIR))
}
...
fn expand_home(path: &str) -> Result<PathBuf> {
    ...
    if let Some(rest) = path.strip_prefix("~/") {
        return Ok(crate::paths::user_home_dir()?.join(rest));
    }
...
impl AgentKind {
    ...
    fn external_tool(self) -> ExternalTool {
        match self {
...
fn detect_agent() -> Option<AgentKind> {
    AGENT_PRIORITY
        .iter()
        .copied()
        .find(|agent| external_tools::available(agent.external_tool()))
}

fn select_permission_mode() -> Result<Option<PermissionMode>> {
    if let Some(mode) = crate::host_actions::test_chef_permission_mode()? {
        return Ok(match mode {
            crate::host_actions::TestChefPermissionMode::FullAuto => Some(PermissionMode::FullAuto),
            crate::host_actions::TestChefPermissionMode::Scoped => Some(PermissionMode::Scoped),
            crate::host_actions::TestChefPermissionMode::Skip => None,
        });
    }
...
fn launch_other_captured(
    ...
    let output = external_tools::command(kind.external_tool())
        .args(&argv_refs)
        .current_dir(project_dir)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
...
async fn launch_claude_streaming(
    ...
    let mut child = external_tools::tokio_command(ExternalTool::ClaudeCode)
        .args(&argv_refs)
        .current_dir(project_dir)
...
fn try_open_frontend(project_dir: &Path) {
    ...
    let reachable = external_tools::command(ExternalTool::Curl)
        .args(["-fsSI", "-m", "1", url])
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
    ...
    match crate::host_actions::open_url(url) {
...
mod tests {
    ...
    fn starter_seed_request_is_write_request() {
        ...
        assert!(request["query"]["write"]["entries"][0]
            .get("for_each")
            .is_some());
    ...
    fn starter_read_request_reads_users() {
        ...
        let root = &request["query"]["read"]["entries"][0]["query"]["root"];

        assert_eq!(request["request_type"], "read");
        assert_eq!(root["value_map"]["input"]["limit"]["count"]["literal"], 25);
        assert_eq!(
            root["value_map"]["input"]["limit"]["input"]["nodes_where"]["predicate"]["eq"]["right"]
                ["constant"]["string"],
            "User"
        );
    }
    ...
    fn agent_prompt_references_current_query_skill_names() {
        ...
        // TypeScript DSL is primary; raw JSON remains the documented fallback.
    ...
    fn agent_prompt_defaults_to_typescript_dsl() {
        ...
        // Stored routes must not be presented as a supported workflow.
        assert!(!prompt.contains("stored query"));
    }
    ...
    #[test]
    fn file_writers_report_filesystem_failures_without_partial_outputs() {
        let root = tempdir().unwrap();
        let not_a_directory = root.path().join("file");
        fs::write(&not_a_directory, "occupied").unwrap();

        assert!(write_agent_prompt(&not_a_directory, None).is_err());
        assert!(write_design_guide(&not_a_directory).is_err());
        assert!(write_example_queries(&not_a_directory).is_err());
        assert_eq!(fs::read_to_string(&not_a_directory).unwrap(), "occupied");
    }

    #[test]
    fn example_query_writer_does_not_partially_write_when_examples_path_is_blocked() {
        let root = tempdir().unwrap();
        fs::write(root.path().join("examples"), "occupied").unwrap();

        assert!(write_example_queries(root.path()).is_err());
        assert_eq!(
            fs::read_to_string(root.path().join("examples")).unwrap(),
            "occupied"
        );
    }

    #[test]
    fn project_snapshot_covers_binary_large_missing_and_normal_files() {
        let root = tempdir().unwrap();
        fs::write(root.path().join("ok.txt"), "hello").unwrap();
        fs::write(root.path().join("binary.bin"), [0xff, 0xfe]).unwrap();
        fs::write(
            root.path().join("large.txt"),
            vec![b'x'; CHEF_SNAPSHOT_MAX_FILE_BYTES as usize + 1],
        )
        .unwrap();

        let (snapshot, files, skipped) = collect_project_snapshot_text(root.path()).unwrap();
        assert!(snapshot.contains("ok.txt"));
        assert_eq!(files.len(), 1);
        assert!(skipped
            .iter()
            .any(|file| { file.path == "binary.bin" && file.reason == "non-utf8 file" }));
        assert!(skipped
            .iter()
            .any(|file| file.path == "large.txt" && file.reason == "file too large"));

        let missing = root.path().join("missing");
        assert!(collect_project_snapshot_text(&missing).is_err());
    }
    ...
    fn run_quietly_restores_verbosity_after_success_and_error() {
        ...
        let mut failure = Box::pin(run_quietly("failure", "never", || async {
            Err(eyre!("expected failure"))
        }));
        let Poll::Ready(Err(error)) = failure.as_mut().poll(&mut context) else {
            panic!("error future should complete immediately");
        };
    ...
    #[test]
    fn chef_metric_covers_success_failure_truncation_and_setup_modes() {
        let success = chef_metric(
            "run",
            "started",
            true,
            Some(1),
            Some(SetupMode::Automatic),
            false,
        ...
        let EventOutcome::Failed { stage, message } = failure.outcome else {
            panic!("expected failed event");
        };
        assert_eq!(stage.as_deref(), Some("agent"));
    ...
    #[test]
    fn format_tool_use_covers_all_named_tools_and_missing_required_fields() {
        for (name, input, expected) in [
            ...
            assert_eq!(format_tool_use(name, &input).as_deref(), Some(expected));
        }
        assert!(format_tool_use("Edit", &serde_json::json!({})).is_none());
        assert!(parse_claude_stream_line("not-json").is_none());
        assert!(parse_claude_stream_line(r#"{"message":"missing type"}"#).is_none());
    }

    #[test]
    fn claude_progress_preserves_multiple_events_and_ignores_non_errors() {
        let event: ClaudeEvent = serde_json::from_value(serde_json::json!({
            "type":"assistant",
            "message":{"content":[
                {"type":"text","text":"hidden"},
                {"type":"tool_use","name":"Read","input":{"file_path":"one.rs"}},
                {"type":"tool_use","name":"Write","input":{"file_path":"two.rs"}},
                {"type":"unknown"}
            ]}
        }))
        .unwrap();
        assert_eq!(
            format_claude_event(&event),
            vec!["📖 Reading one.rs", "✎ Writing two.rs"]
        );
        ...
        .unwrap();
        assert_eq!(format_claude_event(&event), vec!["✗ tool error"]);
    }
