...
pub async fn run(
    ...
    let next_steps = match target {
        InitTarget::Local {
            ...
            config.local.clear();
            let local_config = local_instance_config(port, disk, &s3)?;
            config.local.insert(name, local_config);
...
fn local_instance_config(
    ...
    if s3.has_any() {
        let uri = s3
            .storage_uri
            .as_deref()
            .ok_or_else(|| eyre::eyre!("--storage-uri is required when using S3 storage flags"))?;
...
helix query {instance_name} --file examples/request.json     # send a query JSON request
...
fn append_gitignore(project_dir: &Path) -> Result<()> {
    ...
    let entries = [".helix/", ".env", "target/", "*.log"];
...
mod tests {
    ...
    #[test]
    fn example_request_uses_operation_tree_ast() {
        ...
        assert_eq!(request["query_name"], "node_count");
        assert_eq!(
            root["count"]["input"]["nodes_where"]["predicate"]["eq"]["left"]["property"],
            "$label"
        );
        assert_eq!(
            root["count"]["input"]["nodes_where"]["predicate"]["eq"]["right"]["constant"]["string"],
            "User"
        );
