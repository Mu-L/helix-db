...
async fn sync_enterprise_instance(
    ...
    let config = match project.config.get_instance(instance_name)? {
        ...
        InstanceInfo::Local(_) => {
            return Err(eyre!(
                "Sync is only supported for Enterprise instances; local v2 instances are managed with 'helix start' and query files."
            ));
        }
    };
...
async fn reconcile_enterprise_cluster_snapshot(
    ...
    let apply_pull = || -> Result<()> {
        let query_json_path = pull_remote_enterprise_snapshot_into_local(
            &current_queries_dir,
            &target_queries_dir,
            &remote_manifest,
        )?;
...
fn unique_snapshot_sibling(path: &Path, purpose: &str) -> Result<PathBuf> {
    let parent = path
        .parent()
        .ok_or_else(|| eyre!("Snapshot path has no parent: {}", path.display()))?;
    let name = path
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or_else(|| {
            eyre!(
                "Snapshot path has no valid directory name: {}",
                path.display()
            )
        })?;
...
fn write_remote_snapshot(
    ...
    fs::create_dir_all(target_queries_dir).map_err(|error| {
        eyre!(
            "Failed to create staged enterprise snapshot {}: {error}",
            target_queries_dir.display()
        )
    })?;
    for (relative_path, remote_entry) in remote_manifest {
        ...
        fs::write(&destination, &remote_entry.content)
            .map_err(|error| eyre!("Failed to stage {relative_path}: {error}"))?;
    }
    Ok(())
}

fn restore_snapshot(source: &Path, destination: &Path, errors: &mut Vec<String>) {
...
fn promote_staged_snapshot(
    ...
    target_queries_dir: &Path,
    staged_queries_dir: &Path,
) -> Result<()> {
    let current_is_target = current_queries_dir == target_queries_dir;
    let target_backup = unique_snapshot_sibling(target_queries_dir, "target-backup")?;
    let current_backup = if current_is_target {
        None
    } else {
        Some(unique_snapshot_sibling(
            current_queries_dir,
            "current-backup",
        )?)
    };
    let target_existed = target_queries_dir.exists();
    let current_existed = !current_is_target && current_queries_dir.exists();

    if target_existed {
        fs::rename(target_queries_dir, &target_backup).map_err(|error| {
            eyre!(
                "Failed to prepare existing target snapshot {} for replacement: {error}",
                target_queries_dir.display()
            )
        })?;
    }
    ...
    if current_existed {
        let current_backup = current_backup
            .as_ref()
            .expect("distinct current snapshot always has a backup path");
        if let Err(error) = fs::rename(current_queries_dir, current_backup) {
            let mut rollback_errors = Vec::new();
            if target_existed {
                restore_snapshot(&target_backup, target_queries_dir, &mut rollback_errors);
            }
            let rollback = if rollback_errors.is_empty() {
                String::new()
            } else {
                format!("; rollback also {}", rollback_errors.join("; "))
            };
            return Err(eyre!(
                "Failed to prepare current snapshot {} for replacement: {error}{rollback}",
                current_queries_dir.display()
            ));
        }
    }

    if let Err(error) = fs::rename(staged_queries_dir, target_queries_dir) {
        let mut rollback_errors = Vec::new();
        if current_existed {
            restore_snapshot(
                current_backup
        ...
        return Err(eyre!(
            "Failed to atomically promote enterprise snapshot into {}: {error}{rollback}",
            target_queries_dir.display()
        ));
    }
    ...
    if current_existed {
        ...
        if let Err(error) = fs::remove_dir_all(backup) {
            crate::output::warning(&format!(
                "Enterprise sync succeeded, but the old snapshot backup {} could not be removed: {error}",
                backup.display()
            ));
        }
    }
    if target_existed && let Err(error) = fs::remove_dir_all(&target_backup) {
        crate::output::warning(&format!(
            "Enterprise sync succeeded, but the old target backup {} could not be removed: {error}",
...
fn pull_remote_enterprise_snapshot_into_local(
    ...
    let parent = target_queries_dir.parent().ok_or_else(|| {
        eyre!(
            "Enterprise query snapshot has no parent: {}",
            target_queries_dir.display()
        )
    })?;
    fs::create_dir_all(parent)?;
    let staged_queries_dir = unique_snapshot_sibling(target_queries_dir, "stage")?;

    let result = (|| -> Result<PathBuf> {
        write_remote_snapshot(&staged_queries_dir, remote_manifest)?;
        compile_enterprise_queries(&staged_queries_dir)?;
        promote_staged_snapshot(current_queries_dir, target_queries_dir, &staged_queries_dir)?;
        Ok(target_queries_dir.join("queries.json"))
    })();

    if result.is_err()
        && staged_queries_dir.exists()
        && let Err(error) = fs::remove_dir_all(&staged_queries_dir)
    {
        crate::output::warning(&format!(
            "Failed to remove staged enterprise snapshot {} after sync failure: {error}",
            staged_queries_dir.display()
        ));
    }
...
mod tests {
    ...
    #[test]
    fn compare_manifests_covers_empty_sync_remote_and_tie_states() {
        ...
        let same = HashMap::from([("src/main.rs".to_string(), manifest_entry("same", Some(1)))]);
        assert!(matches!(
            compare_manifests(&same, &same),
            SnapshotComparison::InSync
        ));
        assert!(matches!(
            compare_manifests(&empty, &same),
            SnapshotComparison::RemoteOnly
        ));
        assert!(matches!(
            compare_manifests(&same, &empty),
            SnapshotComparison::LocalOnly
        ));
        ...
        let remote = HashMap::from([("src/main.rs".to_string(), manifest_entry("remote", None))]);
        assert!(matches!(
            compare_manifests(&local, &remote),
            SnapshotComparison::Diverged {
    ...
    #[test]
    fn manifest_diff_and_action_plan_cover_add_update_delete() {
        let local = HashMap::from([
            ("added.rs".to_string(), manifest_entry("a", Some(1))),
            ("changed.rs".to_string(), manifest_entry("local", Some(1))),
        ]);
        let remote = HashMap::from([
            ("removed.rs".to_string(), manifest_entry("r", Some(1))),
            ("changed.rs".to_string(), manifest_entry("remote", Some(1))),
        ]);
        let diff = compute_manifest_diff(&local, &remote);
        assert_eq!(diff.local_only, vec!["added.rs"]);
        assert_eq!(diff.remote_only, vec!["removed.rs"]);
        assert_eq!(diff.changed, vec!["changed.rs"]);

        let push = build_sync_action_plan(&diff, SyncDirection::Push);
        assert_eq!(push.to_create, vec!["added.rs"]);
        assert_eq!(push.to_change, vec!["changed.rs"]);
        assert_eq!(push.to_delete, vec!["removed.rs"]);
        let pull = build_sync_action_plan(&diff, SyncDirection::Pull);
        assert_eq!(pull.to_create, vec!["removed.rs"]);
        assert_eq!(pull.to_delete, vec!["added.rs"]);
    }

    #[test]
    fn remote_paths_cannot_escape_the_query_directory() {
        assert_eq!(
            sanitize_relative_path(Path::new("src/main.rs")).unwrap(),
        ...
        assert!(sanitize_relative_path(Path::new("/absolute.rs")).is_err());
        assert!(sanitize_relative_path(Path::new("src/../../escape.rs")).is_err());
        assert!(sanitize_relative_path(Path::new(".")).is_err());
    }

    #[test]
    fn enterprise_instance_resolution_covers_explicit_empty_and_ambiguous_states() {
...
"#,
        ...
        let project = ProjectContext {
            ...
            config,
            helix_dir: PathBuf::from("/tmp/project/.helix"),
        };
        assert_eq!(
            resolve_enterprise_instance_name(Some("explicit".to_string()), &project).unwrap(),
            "explicit"
        );
        assert!(resolve_enterprise_instance_name(None, &project)
            .unwrap_err()
            .to_string()
            .contains("No Enterprise instances found"));
...
"#,
        ...
        let error = resolve_enterprise_instance_name(None, &project)
            .unwrap_err()
            .to_string();
        assert!(error.contains("Available Enterprise instances: production"));
    }

    #[test]
    fn remote_config_is_accepted_only_when_parseable_and_path_safe() {
        let valid = r#"
[project]
name = "remote"
queries = "./src/queries"
"#;
        let config = parse_and_sanitize_remote_config(valid, "test").unwrap();
        assert_eq!(config.project.queries, Path::new("src/queries"));

        assert!(parse_and_sanitize_remote_config("not = [toml", "test").is_none());
        let unsafe_config = r#"
[project]
name = "remote"
queries = "../escape"
"#;
        assert!(parse_and_sanitize_remote_config(unsafe_config, "test").is_none());
    }

    #[test]
    fn staged_snapshot_promotion_replaces_same_directory_without_partial_files() {
        let root = tempdir().unwrap();
        let target = root.path().join("queries");
        let staged = root.path().join(".queries.stage");
        fs::create_dir_all(&target).unwrap();
        fs::create_dir_all(&staged).unwrap();
        fs::write(target.join("old.rs"), "old").unwrap();
        fs::write(staged.join("new.rs"), "new").unwrap();

        promote_staged_snapshot(&target, &target, &staged).unwrap();

        assert!(!target.join("old.rs").exists());
        assert_eq!(fs::read_to_string(target.join("new.rs")).unwrap(), "new");
    ...
    #[test]
    fn staged_snapshot_promotion_moves_between_directories() {
        let root = tempdir().unwrap();
        let current = root.path().join("queries");
        let target = root.path().join("remote-queries");
        let staged = root.path().join(".remote-queries.stage");
        for directory in [&current, &target, &staged] {
            fs::create_dir_all(directory).unwrap();
        }
        fs::write(current.join("current.rs"), "current").unwrap();
        fs::write(target.join("stale.rs"), "stale").unwrap();
        fs::write(staged.join("remote.rs"), "remote").unwrap();

        promote_staged_snapshot(&current, &target, &staged).unwrap();

        assert!(!current.exists());
        assert!(!target.join("stale.rs").exists());
        assert_eq!(
            fs::read_to_string(target.join("remote.rs")).unwrap(),
            "remote"
        );
    }

    #[test]
    fn failed_promotion_restores_current_and_target_snapshots() {
        let root = tempdir().unwrap();
        let current = root.path().join("queries");
        let target = root.path().join("remote-queries");
        ...
        fs::create_dir_all(&target).unwrap();
        fs::write(current.join("current.rs"), "current").unwrap();
        fs::write(target.join("target.rs"), "target").unwrap();

        let error = promote_staged_snapshot(&current, &target, &missing_stage)
            .unwrap_err()
            .to_string();

        assert!(error.contains("atomically promote"));
        assert_eq!(
            fs::read_to_string(current.join("current.rs")).unwrap(),
            "current"
        );
        assert_eq!(
            fs::read_to_string(target.join("target.rs")).unwrap(),
            "target"
        );
    }

    #[test]
    fn remote_snapshot_write_rejects_unsafe_paths_before_escape() {
        let root = tempdir().unwrap();
        ...
        assert!(write_remote_snapshot(&root.path().join("stage"), &manifest).is_err());
