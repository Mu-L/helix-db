...
#[cfg(test)]
pub(crate) static VERBOSITY_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
...
mod tests {
    ...
    #[test]
    fn operation_step_and_standalone_output_cover_every_verbosity() {
        let _lock = VERBOSITY_TEST_LOCK
            .lock()
        ...
        for verbosity in [
            ...
            Operation::new("Building", "test").success();
            Operation::new("Building", "test").failure();
        ...
        Operation::print_details(&[("URL", "http://localhost")]);
