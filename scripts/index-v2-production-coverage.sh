...
EXCLUSIONS_PATH="$ROOT/scripts/index-v2-production-coverage-exclusions.json"
BASELINES_PATH="$ROOT/scripts/index-v2-production-coverage-baselines.json"
TEMP_ROOT=""
...
jq -e '
    ...
    and all(
        .[];
        (.path
            | startswith("crates/db/src/index_v2/")
                or . == "crates/db/src/encoding/v1/keys/index_v2.rs"
                or startswith("crates/db/src/encoding/v1/values/index_v2/"))
...
case "$#" in
    0)
        ...
        TEMP_ROOT="$(mktemp -d "${TMPDIR:-/private/tmp}/helix-proper-index-v2-coverage.XXXXXX")"
        ...
        (
            ...
            CARGO_TARGET_DIR="$TEMP_ROOT/target" cargo llvm-cov \
                ...
                --test production_index_v2_contracts \
...
SUMMARY="$({
    jq -n \
        ...
        def selected_path($filename):
            ($filename | startswith($root + "/crates/db/src/index_v2/"))
            or ($filename | endswith("/crates/db/src/encoding/v1/keys/index_v2.rs"))
            or ($filename | startswith($root + "/crates/db/src/encoding/v1/values/index_v2/"));
        ...
        | {
            schema_version: 1,
            package: "db",
            coverage_kind: "index-v2-production-source-positions",
