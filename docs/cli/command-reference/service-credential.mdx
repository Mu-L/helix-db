...
Service credentials authenticate headless HTTP API calls and the unified MCP endpoint at
`https://mcp.helix-db.com/mcp`. MCP exposes only the query and customer administration tools allowed
by their project grants; it does not expose Cloud discovery or observability tools to these sessions.
