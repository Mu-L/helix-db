...
`helix start` pulls `ghcr.io/helixdb/helixdb:latest`, starts a background container named `helix-<project>-dev`, publishes port `6969`, and waits until the local server accepts `POST /v2/query` before returning.
