...
Local development runs the prebuilt `ghcr.io/helixdb/helixdb:v0.0.5` container and exposes the standalone server at `POST /v2/query`. By default, storage is in-memory. Use `--disk` when you want persistent local data backed by a CLI-managed MinIO volume.
