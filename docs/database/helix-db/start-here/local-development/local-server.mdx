...
The `ghcr.io/helixdb/helixdb:v0.0.6` image runs the standalone HelixDB server.
...
For an existing project, set `tag = "v0.0.6"` in its `[local.dev]` block in
...
services:
  ...
  helix:
    image: ghcr.io/helixdb/helixdb:v0.0.6
    restart: unless-stopped
    depends_on:
      minio-init:
