...
The engine bounds foreground mutations that affect full-text indexes. These limits
apply to Cloud and standalone HelixDB. Exceeding a bound rejects the write transaction
before commit with HTTP 400 and code `active_text_mutation_limit_exceeded`; it does
...
| --- | ---: |
| Retained/input bytes and text analysis | 64 MiB |
| Output key/value operations | 32,768 |
| Output key/value bytes | 8 MiB |
| Individual split payload and retained split payload budget | 64 MiB |
| Encoded manifest page | 4 MiB |
...
See [Error handling](/database/helix-cloud/operate/error-handling) for the response
