...
never implies query access. The broker uses a 30-second internal capability scoped to the exact
database, gateway audience, and read/write request type. It never creates or uses a customer key.
