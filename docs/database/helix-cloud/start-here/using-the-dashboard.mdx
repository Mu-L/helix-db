...
Set the three values copied from the database page, then send an empty read
request. A successful response confirms the connection.
...
curl --fail-with-body \
  ...
  -H "x-helix-tenant-id: $HELIX_DATABASE_ID" \
  -H "Content-Type: application/json" \
  -d '{"request_type":"read","query":{"read":{"entries":[],"returns":[]}}}'
```
