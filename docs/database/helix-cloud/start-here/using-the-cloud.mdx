...
<Steps>
  ...
  <Step title="Create a database">
    ...
    monthly allowance. Database creation creates a default read-write API key
    and displays its token once. Save it immediately in the application's
    secrets manager; Helix cannot show it again.
  ...
  <Step title="Copy the connection details">
    ...
    **Connect**, copy the endpoint and database ID. To connect an application
    directly to the gateway, use **API Keys** to explicitly create a read-only
    or read-write key and copy its value when it is shown once.
