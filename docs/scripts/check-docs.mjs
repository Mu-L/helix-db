...
if (asStrStart < 0 || asStrEnd < 0) {
  errors.push(
    "crates/ast/src/error_code.rs: cannot locate QueryErrorCode::as_str",
  );
} else {
  const sourceCodes = [
  ...
  const missingCodes = [...sourceSet].filter(
    (code) => !documentedSet.has(code),
  );
  const unknownCodes = [...documentedSet].filter(
    (code) => !sourceSet.has(code),
  );
  if (missingCodes.length > 0) {
    errors.push(
      `error-handling: missing catalog code(s): ${missingCodes.join(", ")}`,
    );
  }
  if (unknownCodes.length > 0) {
    errors.push(
      `error-handling: unknown catalog code(s): ${unknownCodes.join(", ")}`,
    );
  }
}
