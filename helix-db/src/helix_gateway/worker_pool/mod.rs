...
mod tests {
    ...
    use crate::protocol::{Format, request::RequestType};
    use axum::body::Bytes;
    ...
    fn test_worker_pool_with_multiple_cores() {
        ...
        let cores = vec![core_affinity::CoreId { id: 0 }, core_affinity::CoreId { id: 1 }];
        let core_setter = Arc::new(CoreSetter::new(cores, 1));
    ...
    async fn test_route_with_special_characters() {
        let (graph, _temp_dir) = create_test_graph();
        let mut routes = std::collections::HashMap::new();
        routes.insert("query-with-dash".to_string(), Arc::new(test_handler) as Arc<_>);
        routes.insert("query_with_underscore".to_string(), Arc::new(test_handler) as Arc<_>);
        let router = Arc::new(HelixRouter::new(Some(routes), None));
    ...
    async fn test_handler_receives_correct_request() {
        ...
        let (graph, _temp_dir) = create_test_graph();
        let mut routes = std::collections::HashMap::new();
        routes.insert("check_name".to_string(), Arc::new(check_request_handler) as Arc<_>);
        let router = Arc::new(HelixRouter::new(Some(routes), None));
