...
mod tests {
    ...
    fn test_parse_openai_provider_with_model() {
        let result = EmbeddingModelImpl::parse_provider_and_model(Some("openai:text-embedding-3-small"));
        assert!(result.is_ok());
    ...
    fn test_parse_gemini_provider_with_task_type() {
        let result = EmbeddingModelImpl::parse_provider_and_model(Some("gemini:gemini-embedding-001:SEMANTIC_SIMILARITY"));
        assert!(result.is_ok());
        let (provider, model) = result.unwrap();
        match provider {
            EmbeddingProvider::Gemini { task_type } => {
