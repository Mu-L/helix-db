...
impl<'arena> HVector<'arena> {
    ...
    pub fn from_bytes(id: u128, level: usize, bytes: &[u8], arena: &'arena bumpalo::Bump) -> Result<Self, VectorError> {
        if !bytes.len().is_multiple_of(std::mem::size_of::<f64>()){
            return Err(VectorError::InvalidVectorData);
        }

        let mut data = bumpalo::collections::Vec::with_capacity_in(bytes.len() / std::mem::size_of::<f64>(), arena);
